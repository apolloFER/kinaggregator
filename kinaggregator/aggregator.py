...
def _calculate_varint_size(value):
    ...
    num_varint_bytes = int(num_bits_needed / 7)
...
class AggRecord(object):
    ...
    def get_size_bytes(self):
        ...
        return len(kinaggregator.MAGIC) + self._agg_size_bytes + kinaggregator.DIGEST_SIZE
    ...
    def _serialize_to_bytes(self):
        ...
        return kinaggregator.MAGIC + message_body + calculated_digest
    ...
    def add_user_record(self, partition_key, data, explicit_hash_key=None):
        ...
        explicit_hash_key = str(
            explicit_hash_key).strip() if explicit_hash_key is not None else self._create_explicit_hash_key(
            partition_key)
        ...
        if size_of_new_record > kinaggregator.MAX_BYTES_PER_RECORD:
            raise ValueError(
                'Input record (PK=%s, EHK=%s, SizeBytes=%d) is too large to fit inside a single Kinesis record.' %
                (partition_key, explicit_hash_key, size_of_new_record))
        elif self.get_size_bytes() + size_of_new_record > kinaggregator.MAX_BYTES_PER_RECORD:
            return False
