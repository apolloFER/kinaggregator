...
try:
    ...
    print_function("The 'boto3' module is required to run this script. Use 'pip install boto3' to get it.",
                   file=sys.stderr)
    sys.exit(1)
...
def send_record(agg_record):
    ...
    print('Submitting record with EHK=%s NumRecords=%d NumBytes=%d' % (
    ehk, agg_record.get_num_user_records(), agg_record.get_size_bytes()))
    try:
        ...
        print('Transmission Failed: %s' % (e), file=sys.stderr)
    else:
