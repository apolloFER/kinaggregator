...
def install_dependencies():
    ...
    with open(os.devnull, 'w') as devnull:
        ...
        if aws_cmd_line_result != 0:
            print('You do not have "pip" installed or it is not on your PATH.  This script requires access to it.',
                  file=sys.stderr)
            sys.exit(1)
    ...
    for dependency in PIP_DEPENDENCIES:
        ...
        if pip_install_cmd_line_result != 0:
            print('Failed to install module via pip. Try running \'%s\' to debug the issue.' % (pip_install_cmd),
                  file=sys.stderr)
            sys.exit(1)
